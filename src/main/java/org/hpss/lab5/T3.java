...
class T3 {
    public static void run() {
        ...
        MPI.COMM_WORLD.Recv(transitMX, 0, transitMX.length * transitMX[0].length, MPI.INT, rank - 1, 2);

        // Передати до T4: B, MX, Z
        MPI.COMM_WORLD.Send(transitB, H, transitB.length - H, MPI.INT, rank + 1, 3);
        MPI.COMM_WORLD.Send(transitMX, blockSize, transitMX.length * transitMX[0].length - blockSize, MPI.INT, rank + 1, 4);
        ...
        int b3 = maxMatrix(MX_MR_prod);

        // Прийняти b2 від T2
        int b2 = 0;
        MPI.COMM_WORLD.Recv(b3, 0, 1, MPI.INT, rank - 1, 26);

        int bMax13 = Math.max(b2, b3);
        ...
        int a3 = Data.calculateRes(Bh, Ch, Zh, b);
        int a46 = 0;
